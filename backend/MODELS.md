...
### Service (Сервис)
- `serviceId` (String, уникальный): `srv_XXXXXX` - ID сервиса
- `type` (String, обязательный): Тип сервиса (`whatsapp`, `telegram`, `viber`, `sms`)
- `apiUrl` (String, обязательный): URL API для отправки сообщений
- `isActive` (Boolean): Активен ли сервис (по умолчанию `true`)
- `description` (String, опциональное): Описание сервиса
- `createdAt` (Date): Дата создания
- `updatedAt` (Date): Дата обновления

**Описание:** Сервисы - это конфигурация внешних API для отправки исходящих сообщений бизнес-контактам. Каждый сервис содержит URL API, куда нужно отправлять сообщения для конкретного типа канала.

**Ограничения:**
- Индекс на `type` (для быстрого поиска сервиса по типу)
...
- Service - независимая модель, не привязана к аккаунтам (глобальная конфигурация)
