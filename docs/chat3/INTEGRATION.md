...
#### 🎯 Персонализация P2P-диалогов

- Для каждого участника личного диалога сохраняйте персональные метки:
  - `p2pDialogNameFor<userId>` — строка с именем собеседника, отображаемым этому пользователю (участвует в поиске).
  - `p2pDialogAvatarFor<userId>` — URL-адрес аватарки собеседника (опционально).
- Запись выполняется через `PUT /meta/dialog/{dialogId}/{key}`. Пример:
  ```
  PUT /meta/dialog/dlg_123/p2pDialogNameForusr_a1
  { "value": "Петров Петр" }
  ```
- Поиск `GET /users/{userId}/dialogs` поддерживает фильтер `meta.p2pDialogNameFor<userId>`:\
  `filter=(meta.type,eq,p2p)&(meta.p2pDialogNameForusr_a1,regex,".*иван.*")`
- При изменении профиля участника (имя, фото) интеграция должна оперативно обновить соответствующие мета-теги.
